...
ring_buffer *_rx_buffer1 = &rx_buffer1;
ring_buffer *_tx_buffer1 = &tx_buffer1;
ring_buffer *_rx_buffer2 = &rx_buffer2;
ring_buffer *_tx_buffer2 = &tx_buffer2;
...
{
  /* Enable the UART & SPI Error Interrupt: (Frame error, noise error, overrun error) */
  __HAL_UART_ENABLE_IT(device_uart, UART_IT_ERR);
  __HAL_UART_ENABLE_IT(pc_uart, UART_IT_ERR);
  __HAL_SPI_ENABLE_IT(wifi_spi, SPI_IT_ERR);

  /* Enable the UART & SPI Data Register not empty Interrupt */
  __HAL_UART_ENABLE_IT(device_uart, UART_IT_RXNE);
  __HAL_UART_ENABLE_IT(pc_uart, UART_IT_RXNE);
  __HAL_SPI_ENABLE_IT(wifi_spi, SPI_IT_RXNE);
}
...
int spi_read(SPI_HandleTypeDef *spi)
{
	if (spi == wifi_spi)
	{
		  // if the head isn't ahead of the tail, we don't have any characters
...
void spi_write(int c, SPI_HandleTypeDef *spi)
{
	if (c>=0)
	{
		if (spi == wifi_spi){
		...
		__HAL_SPI_ENABLE_IT(wifi_spi, SPI_IT_TXE); // Enable UART transmission interrupt
		}
	}
}

int IsUartDataAvailable(UART_HandleTypeDef *uart)
{
...
int IsSpiDataAvailable(SPI_HandleTypeDef *spi)
{
	if (spi == wifi_spi) return (uint16_t)(UART_BUFFER_SIZE + _rx_buffer1->head - _rx_buffer1->tail) % UART_BUFFER_SIZE;
	return -1;
}
...
int Get_after (char *string, uint8_t numberofchars, char *buffertosave, UART_HandleTypeDef *uart)
{
	...
	{
		while (!(IsUartDataAvailable(uart)));
...
again_device:
	timeout = TIMEOUT_DEF;
	while ((!IsUartDataAvailable(uart))&&timeout); 
...
void my_Uart_isr (UART_HandleTypeDef *huart)
{
...
void my_spi_isr (SPI_HandleTypeDef *hspi) 
{
	  uint32_t isrflags   = READ_REG(hspi->Instance->SR);
	  uint32_t cr1its     = READ_REG(hspi->Instance->CR2);

    /* if DR is not empty and the Rx Int is enabled */
    if (((isrflags & SPI_SR_RXNE) != RESET) && ((cr1its & SPI_CR2_RXNEIE) != RESET))
    {
    	 /******************
    	    	      *  @note   PE (Parity error), FE (Framing error), NE (Noise error), ORE (Overrun
    	    	      *          error) and IDLE (Idle line detected) flags are cleared by software
    	 ...
    	 *********************/
		hspi->Instance->SR;                       /* Read status register */
        unsigned char c = hspi->Instance->DR;     /* Read data register */
        if (hspi == wifi_spi)
        {
        	store_char (c, _rx_buffer1);  // store data in buffer
        }
    ...
    /*If interrupt is caused due to Transmit Data Register Empty */
    if (((isrflags & SPI_SR_TXE) != RESET) && ((cr1its & SPI_CR2_TXEIE) != RESET))
    {
    	if (hspi == wifi_spi){
    	if(tx_buffer1.head == tx_buffer1.tail)
    	    {
    	 ...
    	 else
    	    {
			  ...
			  hspi->Instance->CR1 |= (1<<6); //SPI Enable
			  unsigned char *c;
			  volatile uint16_t view;

			  // There is more data in the output buffer. Send the next byte
    	      if((tx_buffer1.head - tx_buffer1.tail) > 1U)
			  {
			  c = &tx_buffer1.buffer[tx_buffer1.tail];
    	      tx_buffer1.tail = (tx_buffer1.tail + 2U) % UART_BUFFER_SIZE;
			  hspi->Instance->DR = *((uint16_t *)c);
			  //view = *((uint16_t *)c);
			  }
			  ...
			  {
				c = &tx_buffer1.buffer[tx_buffer1.tail];
    	        tx_buffer1.tail = (tx_buffer1.tail + 1U) % UART_BUFFER_SIZE;
				*((__IO uint8_t *)&hspi->Instance->DR) = (*c);
				view = *((uint16_t *)c);
			  }
    	      /******************
    	      *  @note   PE (Parity error), FE (Framing error), NE (Noise error), ORE (Overrun
    	      ...
    	      *********************/
    	      	...
    	      	while (!((hspi->Instance->SR)&(1<<1))) {};  // wait for TXE bit to set -> This will indicate that the buffer is empty
				while (((hspi->Instance->SR)&(1<<7))) {};  // wait for BSY bit to Reset -> This will indicate that SPI is not busy in communication	

  				while (((hspi->Instance->SR)&(1<<0))) 
				{
					uint8_t temp = hspi->Instance->DR;
				}; 
				
				uint8_t	temp1 = hspi->Instance->SR;
